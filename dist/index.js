...
bot.on('message', async (msg) => {
    ...
    if (msg.photo) {
        ...
        try {
            ...
            const filePath = await downloadImage(fileUrl, fileInfo.file_path);
            const submissionId = await submitToAstrometry(filePath);
            if (!submissionId) {
            ...
            const result = await getAstrometryResult(submissionId);
            if (result) {
...
async function submitToAstrometry(filePath) {
    try {
...
async function getAstrometryResult(submissionId) {
    let jobId = null;
